...
class CL_Sweep_Controller(gr.sync_block):
    """
Block to sweep gain and measure 2 quantities. Used to measure conversion loss curves The block functions by publishing a message with the variable to be swept 
which should be connected to a "Message Pair to Var" block. The 2 inputs are recorded for each 
